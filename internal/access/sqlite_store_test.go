...
func TestSQLiteTargetStore_CreateWithPath_Validation(t *testing.T) {
	...
	if _, err := targets.CreateWithPath(ctx, "", "n", "h", 22, ProtocolSSH, GroupID("p"), "p", "", "", "", "", true, false, false); err == nil {
		t.Fatalf("expected error for empty id, got nil")
	}
	if _, err := targets.CreateWithPath(ctx, "id", "", "h", 22, ProtocolSSH, GroupID("p"), "p", "", "", "", "", true, false, false); err == nil {
		t.Fatalf("expected error for empty name, got nil")
	}
	if _, err := targets.CreateWithPath(ctx, "id", "n", "", 22, ProtocolSSH, GroupID("p"), "p", "", "", "", "", true, false, false); err == nil {
		t.Fatalf("expected error for empty host, got nil")
	}
	// invalid groupID (when non-empty): invalid characters
	if _, err := targets.CreateWithPath(ctx, "id", "n", "h", 22, ProtocolSSH, GroupID("g 1"), "g 1", "", "", "", "", true, false, false); err == nil {
...
func TestSQLiteTargetStore_Update(t *testing.T) {
	...
	_, _ = targets.CreateWithPath(ctx, "t1", "old", "10.0.0.1", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false)

	updated, err := targets.Update(ctx, "t1", "new", "10.0.0.2", 2222, ProtocolTelnet, "g1/sub", "user", "", "", "", false, false, false)
	if err != nil {
	...
	if _, err := targets.Update(ctx, "missing", "n", "h", 22, ProtocolSSH, "", "", "", "", "", true, false, false); err != ErrTargetNotFound {
...
func TestSQLiteTargetStore_Get_NotFoundAndPortBounds(t *testing.T) {
	...
	_, _ = groups.Create(ctx, "g1", "G1")
	if _, err := targets.CreateWithPath(ctx, "t1", "ok", "127.0.0.1", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false); err != nil {
...
func TestSQLiteTargetStore_SSHPassword_EncryptionRequiredWithoutKey(t *testing.T) {
	...
	groups, targets := newTestSQLiteStores(t)
	_, _ = groups.Create(ctx, "g1", "G1")
	_, err := targets.CreateWithPath(ctx, "t1", "n", "127.0.0.1", 22, ProtocolSSH, GroupID("g1"), "g1", "user", "secret", "", "", true, false, false)
...
func TestSQLiteTargetStore_SSHPassword_EncryptDecryptWithKey(t *testing.T) {
	...
	_, _ = groups.Create(ctx, "g1", "G1")
	_, err := storeWithKey.CreateWithPath(ctx, "t1", "n", "127.0.0.1", 22, ProtocolSSH, GroupID("g1"), "g1", "root", "mypass", "", "", true, false, false)
	if err != nil {
...
func TestSQLiteTargetStore_ListByIDs_SkipsInvalidAndDuplicates(t *testing.T) {
	...
	_, _ = groups.Create(ctx, "g1", "G1")
	_, _ = targets.CreateWithPath(ctx, "t1", "A", "10.0.0.1", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false)
	_, _ = targets.CreateWithPath(ctx, "t2", "B", "10.0.0.2", 23, ProtocolTelnet, GroupID("g1"), "g1", "", "", "", "", false, false, false)
...
func TestSQLiteAccessGroupStore_KeysetPagination(t *testing.T) {
	...
	_, _ = groups.Create(ctx, "g1", "G1")
	_, _ = targets.CreateWithPath(ctx, "t1", "A", "10.0.0.1", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false)
	_, _ = targets.CreateWithPath(ctx, "t2", "B", "10.0.0.2", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false)
	_, _ = targets.CreateWithPath(ctx, "t3", "C", "10.0.0.3", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false)
	_, _ = targets.CreateWithPath(ctx, "t4", "D", "10.0.0.4", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false)
...
func TestSQLiteAccessGroupStore_TagBasedAccess(t *testing.T) {
	...
	_, _ = groups.Create(ctx, "g1", "G1")
	_, _ = targets.CreateWithPath(ctx, "t1", "T1", "h1", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false)
	_ = groups.AddTargetToGroup(ctx, "g1", "t1")
...
func TestSQLiteAccessGroupStore_GroupIDsForUser_ViaTag(t *testing.T) {
	...
	_, _ = targets.CreateWithPath(ctx, "t2", "T2", "h2", 22, ProtocolSSH, GroupID("g2"), "g2", "", "", "", "", true, false, false)
...
func TestSQLiteTargetStore_CreateWithPath_InvalidProtocolAndHost(t *testing.T) {
	...
	if _, err := targets.CreateWithPath(ctx, "t1", "n", "127.0.0.1", 22, "invalid", GroupID("g1"), "g1", "", "", "", "", false, false, false); err == nil {
		t.Fatal("expected error for invalid protocol")
	}
	if _, err := targets.CreateWithPath(ctx, "t1", "n", "", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false); err == nil {
		t.Fatal("expected error for empty host")
	}
	// hostname must not start/end with hyphen or dot per pattern
	if _, err := targets.CreateWithPath(ctx, "t1", "n", "-bad", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false); err == nil {
		t.Fatal("expected error for invalid hostname (-bad)")
	}
	if _, err := targets.CreateWithPath(ctx, "t1", "n", string(make([]byte, 254)), 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false); err == nil {
...
func TestSQLiteTargetStore_CreateWithPath_InvalidTargetID(t *testing.T) {
	...
	if _, err := targets.CreateWithPath(ctx, "", "n", "127.0.0.1", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false); err == nil {
		t.Fatal("expected error for empty target id")
	}
	if _, err := targets.CreateWithPath(ctx, TargetID(string(make([]byte, 513))), "n", "127.0.0.1", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false); err == nil {
		t.Fatal("expected error for target id too long")
	}
	if _, err := targets.CreateWithPath(ctx, "bad id!", "n", "127.0.0.1", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false); err == nil {
		t.Fatal("expected error for invalid target id characters")
	}
	// invalid group_id when non-empty
	if _, err := targets.CreateWithPath(ctx, "t1", "n", "127.0.0.1", 22, ProtocolSSH, GroupID("bad!"), "p", "", "", "", "", true, false, false); err == nil {
...
func TestSQLiteTargetStore_TagsForTarget_SetTargetTags(t *testing.T) {
	...
	groups, targets := newTestSQLiteStores(t)
	_, _ = groups.Create(ctx, "g1", "G1")
	_, _ = targets.CreateWithPath(ctx, "t1", "H1", "10.0.0.1", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false)
...
func TestSQLiteAccessGroupStore_AddTargetToGroup_CanceledContext(t *testing.T) {
	groups, targets := newTestSQLiteStores(t)
	_, _ = groups.Create(context.Background(), "g1", "G1")
	_, _ = targets.CreateWithPath(context.Background(), "t1", "T", "127.0.0.1", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false)
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
...
func TestSQLiteTargetStore_Update_Validation(t *testing.T) {
	...
	if _, err := targets.Update(ctx, "", "n", "h", 22, ProtocolSSH, "", "", "", "", "", true, false, false); err == nil {
		t.Fatal("expected error for empty id")
	}
	if _, err := targets.Update(ctx, "t1", "", "h", 22, ProtocolSSH, "", "", "", "", "", true, false, false); err == nil {
		t.Fatal("expected error for empty name")
	}
	if _, err := targets.Update(ctx, "t1", "n", "", 22, ProtocolSSH, "", "", "", "", "", true, false, false); err == nil {
		t.Fatal("expected error for empty host")
	}
	if _, err := targets.Update(ctx, "t1", "n", "h", 22, Protocol("invalid"), "", "", "", "", "", true, false, false); err == nil {
		t.Fatal("expected error for invalid protocol")
	}
	if _, err := targets.Update(ctx, "t1", "n", "h", 22, ProtocolSSH, "", "", "secret", "", "", true, false, false); err != ErrEncryptionKeyRequired {
...
func TestSQLiteTargetStore_Update_CanceledContext(t *testing.T) {
	groups, targets := newTestSQLiteStores(t)
	_, _ = groups.Create(context.Background(), "g1", "G1")
	_, _ = targets.CreateWithPath(context.Background(), "t1", "N", "127.0.0.1", 22, ProtocolSSH, GroupID("g1"), "g1", "", "", "", "", true, false, false)
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	_, err := targets.Update(ctx, "t1", "n", "127.0.0.1", 22, ProtocolSSH, "", "", "", "", "", true, false, false)
	if err == nil {
		t.Fatal("expected error with canceled context")
	}
}
...
func TestSQLiteTargetStore_Get_DecryptFailure(t *testing.T) {
	...
	storeWithKey := NewSQLiteTargetStore(targets.db, nil, key)
	_, _ = groups.Create(ctx, "g1", "G1")
	_, err := storeWithKey.CreateWithPath(ctx, "t1", "N", "127.0.0.1", 22, ProtocolSSH, GroupID("g1"), "g1", "u", "pass", "", "", true, false, false)
	if err != nil {
		t.Fatalf("CreateWithPath: %v", err)
	}

...
func TestRunBridgeDetachable_WithEchoServer(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		go func() {
			...
			defer cancel()
			err := RunBridgeDetachable(ctx, "127.0.0.1", port, "", "", output, attachCh, wsConn, nil, nil, nil, 80, 24)
			bridgeErrCh <- err
		}()
	}))
	defer srv.Close()
...
func TestRunBridgeDetachable_StreamAttach(t *testing.T) {
	...
	go func() {
		...
		defer cancel()
		bridgeErrCh <- RunBridgeDetachable(ctx, "127.0.0.1", port, "", "", output, attachCh, nil, nil, nil, nil, 80, 24)
	}()

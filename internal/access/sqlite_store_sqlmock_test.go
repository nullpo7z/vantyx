...
func TestSQLiteTargetStore_Update_Success(t *testing.T) {
	...
	mock.ExpectQuery("SELECT id, name, host, port, protocol, path").
		WithArgs("t1").
		WillReturnRows(
			sqlmock.NewRows([]string{"id", "name", "host", "port", "protocol", "path", "ssh_username", "ssh_password", "ssh_private_key", "ssh_private_key_passphrase", "sftp_enabled", "ftp_enabled", "tftp_enabled", "ssh_host_key_fingerprint", "ssh_host_key_insecure_skip_verify"}).

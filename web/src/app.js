...
export function renderApp(container) {
  ...
  function showTargetsList(list, errMsg) {
    if (errMsg) {
      mainContent.innerHTML = `
        <div class="w-full mb-6">
          <h2 class="text-xl font-semibold text-slate-800">ホーム</h2>
        </div>
        <div class="w-full bg-white rounded-lg shadow-sm border border-slate-200 p-6 text-center">
          <p class="text-sm text-red-600">${escapeHtml(errMsg)}</p>
        </div>
      `
      ...
      mainContent.innerHTML = `
        ...
        <div class="w-full bg-white rounded-lg shadow-sm border border-slate-200 p-6 text-center">
          <p class="text-slate-500">アクセス可能なターゲットがありません。</p>
        </div>
      `
    } else {
      mainContent.innerHTML = `
        ...
        <div id="targets-list" class="w-full space-y-6"></div>
      `
      ...
      Object.keys(groups)
        ...
        .forEach((path) => {
          ...
          const title = path ? path : 'root'
          section.innerHTML = `
            <div class="flex items-center justify-between mb-2">
              <h3 class="text-sm font-semibold text-slate-700">${escapeHtml(title)}</h3>
              <span class="text-xs text-slate-500">${groups[path].length} 件</span>
            </div>
            <div class="space-y-2" data-path="${escapeHtml(path)}"></div>
          `
          const listEl = section.querySelector('div.space-y-2')
          groups[path]
            .slice()
            .sort((x, y) => (x.name || '').localeCompare(y.name || ''))
            .forEach((t) => {
              const card = document.createElement('div')
              card.className = 'bg-white rounded-lg border border-slate-200 p-4 flex items-center justify-between shadow-sm'
              card.innerHTML = `
                <div>
                  <span class="font-medium text-slate-800">${escapeHtml(t.name)}</span>
                  <span class="text-slate-500 text-sm ml-2">${escapeHtml(t.host)}:${t.port} (${t.protocol})</span>
                </div>
                <button data-target-id="${escapeHtml(t.id)}" data-target-name="${escapeHtml(t.name)}" data-protocol="${escapeHtml(t.protocol)}"
                  class="connect-btn rounded bg-sky-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-sky-700 shadow-sm transition-colors disabled:opacity-50">
                  接続
                </button>
              `
              listEl.appendChild(card)
            })
          targetsList.appendChild(section)
        })
      mainContent.querySelectorAll('.connect-btn').forEach((btn) => {
        btn.addEventListener('click', () => {
          const targetId = btn.dataset.targetId
          const targetName = btn.dataset.targetName
          const protocol = btn.dataset.protocol
  ...
  function groupByPath(targets) {
    const out = {}
      ; (targets || []).forEach((t) => {
        // Back-end sets target.path to group_id when not explicitly provided.
        const p = (t.path || '').trim()
        if (!out[p]) out[p] = []
        out[p].push(t)
      })
  ...
  function getChildGroups(groups, parentId) {
    const out = []
      ; (groups || []).forEach((g) => {
        const id = (g.id || '').trim()
        if (!id) return
        ...
        if (!parentId) {
          if (parts.length === 1) out.push(g)
          return
        }
        if (!id.startsWith(parentId + '/')) return
        const rest = id.slice(parentId.length + 1)

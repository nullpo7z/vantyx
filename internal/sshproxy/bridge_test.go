...
func TestRunBridge_TouchCalled(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		go func() {
			...
			_ = RunBridge(ctx, wsConn, "127.0.0.1", port, "test", "test", touch)
		}()
	}))
	defer srv.Close()
...
func TestRunBridge_ContextCancelReturns(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		go func() {
			...
			bridgeErr = RunBridge(ctx, wsConn, "127.0.0.1", port, "test", "test", nil)
			close(done)
		}()
	}))
	defer srv.Close()
...
func runBridgeWithEchoServer(t *testing.T) (wsURL string, bridgeErrCh chan error, sshServer *mock.SSHEchoServer) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		go func() {
			...
			ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
			defer cancel()
			err := RunBridge(ctx, wsConn, "127.0.0.1", port, "test", "test", nil)
			bridgeErrCh <- err
		}()
	}))
	t.Cleanup(srv.Close)
...
func TestRunBridge_GoroutineErrorPaths(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		go func() {
			...
			defer cancel()
			bridgeErrCh <- RunBridge(ctx, wsConn, "127.0.0.1", 22, "u", "p", nil)
		}()
	}))
	defer srv.Close()
	u, _ := url.Parse(srv.URL)
...
func TestRunBridge_WriteMessageFails(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		go func() {
			ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
			defer cancel()
			bridgeErrCh <- RunBridge(ctx, conn, "127.0.0.1", 22, "u", "p", nil)
			_ = conn.Close()
		}()

...
func TestSQLiteAccessGroupStore_GroupIDsForUser_Query1Error(t *testing.T) {
	...
	wantErr := errors.New("query failed")
	mock.ExpectQuery("SELECT group_id FROM user_groups WHERE user_id").WithArgs("u1", maxFanoutRowsPerQuery).WillReturnError(wantErr)
...
func TestSQLiteAccessGroupStore_GroupIDsForUser_Rows1Err(t *testing.T) {
	...
	mock.ExpectQuery("SELECT group_id FROM user_groups WHERE user_id").WithArgs("u1", maxFanoutRowsPerQuery).WillReturnRows(rows)
...
func TestSQLiteAccessGroupStore_GroupIDsForUser_Query2Error(t *testing.T) {
	...
	mock.ExpectQuery("SELECT group_id FROM user_groups WHERE user_id").WithArgs("u1", maxFanoutRowsPerQuery).WillReturnRows(sqlmock.NewRows([]string{"group_id"}))
...
func TestSQLiteAccessGroupStore_GroupIDsForUser_Query3Error(t *testing.T) {
	...
	mock.ExpectQuery("SELECT DISTINCT gtag.group_id").WithArgs("u1", maxFanoutRowsPerQuery).WillReturnError(wantErr)
...
func TestSQLiteAccessGroupStore_GroupIDsForUser_Rows3Err(t *testing.T) {
	...
	mock.ExpectQuery("SELECT group_id FROM user_groups WHERE user_id").WithArgs("u1", maxFanoutRowsPerQuery).WillReturnRows(sqlmock.NewRows([]string{"group_id"}))
	mock.ExpectQuery("SELECT DISTINCT gt.group_id").WithArgs("u1", maxFanoutRowsPerQuery).WillReturnRows(sqlmock.NewRows([]string{"group_id"}))
	mock.ExpectQuery("SELECT DISTINCT gtag.group_id").WithArgs("u1", maxFanoutRowsPerQuery).WillReturnRows(rows)
...
func TestSQLiteTargetStore_Update_Success(t *testing.T) {
	...
	mock.ExpectQuery("SELECT id, name, host, port, protocol, path").
		WithArgs("t1").
		WillReturnRows(
			sqlmock.NewRows([]string{"id", "name", "host", "port", "protocol", "path", "ssh_username", "ssh_password", "ssh_private_key", "ssh_private_key_passphrase", "sftp_enabled", "ftp_enabled", "tftp_enabled", "ssh_host_key_fingerprint", "ssh_host_key_insecure_skip_verify", "credential_identity_id", "ssh_key_id"}).
				AddRow("t1", "new", "10.0.0.2", 2222, "telnet", "path", "user", "", "", "", 1, 0, 0, "", 0, "", ""))

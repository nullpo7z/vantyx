...
- **証跡ログの高度な検索**: 監査画面で監査イベント・コマンドログを期間・ユーザー・ターゲットで検索。録画画面で期間・チャネル絞り込み（管理者は他ユーザーの録画も `user_id` で参照可）
...
### 証跡・ログ（Phase 3 の一部）
- **証跡ログ検索の拡張**: カーソルページング、コマンドログの完全なシェル履歴保証、監査ログ SIEM 転送の運用強化（設定 UI はある）
...
| 録画・証跡         | Asciinema 録画、一覧・再生、期間/ユーザー/ターゲット検索、監査ログ DB | ページング、完全シェル履歴               |

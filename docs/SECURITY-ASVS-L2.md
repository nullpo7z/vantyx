...
| SSH host key TOFU | Adopting / clearing the SSH host key fingerprint is admin-only and audited (`target_host_key_probed`, `target_host_key_adopted`, `target_host_key_cleared`). Probing dials with `WithInsecureSkipHostKeyVerify` for one connection only and routes the captured `SHA256:` fingerprint through `WithCapturedFingerprint` without persisting anything. Stored fingerprints are validated as `SHA256:<base64>` (no padding, OpenSSH default) before update. Mismatch on connect aborts the bridge and surfaces a structured `host_key_mismatch` WebSocket frame; the SPA forces a checkbox confirmation before adopting the new key. The legacy `VANTYX_SSH_INSECURE_IGNORE_HOST_KEY=1` env opt-out is unchanged. | `internal/sshproxy/hostkey.go`, `internal/httpapi/targets_hostkey.go`, `internal/httpapi/terminal_hostkey_frame.go` |

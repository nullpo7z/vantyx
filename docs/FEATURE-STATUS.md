...
- **証跡ログの高度な検索**: 監査画面で監査イベント・コマンドログを期間・ユーザー・ターゲットで検索。録画画面で期間・チャネル絞り込み（管理者は他ユーザーの録画も `user_id` で参照可）。監査ログは `after_id` / `next_cursor` によるキーセットページング対応
...
| 録画・証跡         | Asciinema 録画、一覧・再生、期間/ユーザー/ターゲット検索、監査ログ DB・ページング | 完全シェル履歴、コマンドログページング   |

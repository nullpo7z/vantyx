...
func passphraseProtectedOpenSSHKey(passphrase []byte) openSSHDecryptFunc {
	return func(cipherName, kdfName, kdfOpts string, privKeyBlock []byte) ([]byte, error) {
		...
		keyLen := 32
		ivLen := 16
		if cipherName == "aes256-gcm@openssh.com" {
			ivLen = 12 // GCM nonce
		}
		k, err := bcrypt_pbkdf.Key(passphrase, []byte(opts.Salt), int(opts.Rounds), keyLen+ivLen)
		if err != nil {
			return nil, err
		}
		key, iv := k[:keyLen], k[keyLen:]
		...
		case "aes256-gcm@openssh.com":
			...
			plain, err := aead.Open(nil, iv, privKeyBlock, nil)
			if err != nil {
				return nil, err
			}
			return plain, nil
		default:
			...
			default:
				return nil, fmt.Errorf("ssh: unknown cipher %q, only supports %q, %q or %q", cipherName, "aes256-ctr", "aes256-cbc", "aes256-gcm@openssh.com")
			}
		}
